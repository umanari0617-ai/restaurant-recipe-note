...
function migrateRecipes(x){
  ...
  x.recipes.forEach(r=>{
    ...
    if(!r.prepMultiplier||!SERVING_MULT[r.prepMultiplier])r.prepMultiplier="1回分";
...
function bind(){
  ...
  $("lockButton").onclick=toggleManagerMode;
...
function updateManagerUI(){
  $("lockButton").textContent=isManagerMode?"🔓":"🔒";
...
function showView(name){
  ...
  document.querySelector(".tab-bar").classList.toggle("hidden",!isTopLevel);
...
function recipeRowHtml(r){
  ...
  const meta=r.needsPrep?`<span class="recipe-row-mult">${esc(r.prepMultiplier)}</span>`:"";
...
function openPrepPickModal(r){
  ...
  prepPickValue=r.prepMultiplier||"1回分";
  $("prepPickName").textContent=r.name;
  renderPrepPickOptions();
  openModal("prepPick");
}
function renderPrepPickOptions(){
  $("prepPickOptions").innerHTML=SERVING_OPTIONS.map(o=>`<span data-v="${o}" class="${o===prepPickValue?"active":""}">${o}</span>`).join("");
...
function openRecipeDetail(rid){
  ...
  $("detailPrepMultiplier").innerHTML=SERVING_OPTIONS.map(o=>`<option value="${o}">${o}</option>`).join("");
  $("detailPrepMultiplier").value=r.prepMultiplier||"1回分";
...
function renderDetailNotes(r){
  ...
  $("detailNotes").innerHTML=notes.map(n=>`
    <div class="note-item">
      <div class="note-item-head"><span>${esc(n.name||"名無し")}</span><span>${esc(n.date)}</span></div>
...
function addNote(){
  ...
  r.notes=r.notes||[];
  r.notes.unshift({name,text,date:todayStr()});
  save();
  renderDetailNotes(r);
  $("newNoteName").value="";
  $("newNoteText").value="";
...
function saveRecipeForm(e){
  ...
  if(existingId){
    ...
    state.recipes.push({id:id(),needsPrep:false,prepMultiplier:"1回分",prepCount:0,lastPreppedDate:"",notes:[],...data});
  }
  save();
